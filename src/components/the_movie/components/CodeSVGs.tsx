import React from 'react'
import { AnyExoticRefComponent } from '../types.interface'

export const AngularSVG: AnyExoticRefComponent<SVGSVGElement> = React.forwardRef(((_props: any, ref: React.RefObject<SVGSVGElement> | ((instance: SVGSVGElement) => void) | null): JSX.Element => (
  <svg viewBox="0 0 32 32" width="128" height="128" ref={ref}>
    <polygon points="16 2 16 2 16 2 2.966 6.648 4.954 23.882 16 30 16 30 16 30 27.046 23.882 29.034 6.648 16 2"
      fill="#dd0031"
      stroke="#dd0031"
      strokeWidth=".25"
    />
    <polygon points="16 2 16 5.108 16 5.094 16 19.276 16 19.276 16 30 16 30 27.046 23.882 29.034 6.648 16 2"
      fill="#c3002f"
      stroke="#c3002f"
      strokeWidth=".25"
    />
    <path d="M16,5.094,7.852,23.364H10.89l1.638-4.088h6.916l1.638,4.088H24.12L16,5.094Zm2.38,11.662H13.62L16,11.03Z"
      fill="#FFF"
      stroke="#FFF"
      strokeWidth=".25"
    />
  </svg>
)))

export const ReactSVG: AnyExoticRefComponent<SVGSVGElement> = React.forwardRef(((_props: any, ref: React.RefObject<SVGSVGElement> | ((instance: SVGSVGElement) => void) | null): JSX.Element => (
  <svg viewBox="0 0 32 32" width="128" height="128" ref={ref}>
    <circle
      cx="16"
      cy="15.974"
      r="2.5"
      fill="#00d8ff"
      stroke="#00d8ff"
      strokeWidth=".25"
    />
    <path
      d="M16,21.706a28.385,28.385,0,0,1-8.88-1.2,11.3,11.3,0,0,1-3.657-1.958A3.543,3.543,0,0,1,2,15.974c0-1.653,1.816-3.273,4.858-4.333A28.755,28.755,0,0,1,16,10.293a28.674,28.674,0,0,1,9.022,1.324,11.376,11.376,0,0,1,3.538,1.866A3.391,3.391,0,0,1,30,15.974c0,1.718-2.03,3.459-5.3,4.541A28.8,28.8,0,0,1,16,21.706Zm0-10.217a27.948,27.948,0,0,0-8.749,1.282c-2.8.977-4.055,2.313-4.055,3.2,0,.928,1.349,2.387,4.311,3.4A27.21,27.21,0,0,0,16,20.51a27.6,27.6,0,0,0,8.325-1.13C27.4,18.361,28.8,16.9,28.8,15.974a2.327,2.327,0,0,0-1.01-1.573,10.194,10.194,0,0,0-3.161-1.654A27.462,27.462,0,0,0,16,11.489Z"
      fill="#00d8ff"
      stroke="#00d8ff"
      strokeWidth=".25"
    />
    <path
      d="M10.32,28.443a2.639,2.639,0,0,1-1.336-.328c-1.432-.826-1.928-3.208-1.327-6.373a28.755,28.755,0,0,1,3.4-8.593h0A28.676,28.676,0,0,1,16.71,5.995a11.376,11.376,0,0,1,3.384-2.133,3.391,3.391,0,0,1,2.878,0c1.489.858,1.982,3.486,1.287,6.859a28.806,28.806,0,0,1-3.316,8.133,28.385,28.385,0,0,1-5.476,7.093,11.3,11.3,0,0,1-3.523,2.189A4.926,4.926,0,0,1,10.32,28.443Zm1.773-14.7a27.948,27.948,0,0,0-3.26,8.219c-.553,2.915-.022,4.668.75,5.114.8.463,2.742.024,5.1-2.036a27.209,27.209,0,0,0,5.227-6.79,27.6,27.6,0,0,0,3.181-7.776c.654-3.175.089-5.119-.713-5.581a2.327,2.327,0,0,0-1.868.089A10.194,10.194,0,0,0,17.5,6.9a27.464,27.464,0,0,0-5.4,6.849Z"
      fill="#00d8ff"
      stroke="#00d8ff"
      strokeWidth=".25"
    />
    <path
      d="M21.677,28.456c-1.355,0-3.076-.82-4.868-2.361a28.756,28.756,0,0,1-5.747-7.237h0a28.676,28.676,0,0,1-3.374-8.471,11.376,11.376,0,0,1-.158-4A3.391,3.391,0,0,1,8.964,3.9c1.487-.861,4.01.024,6.585,2.31a28.8,28.8,0,0,1,5.39,6.934,28.384,28.384,0,0,1,3.41,8.287,11.3,11.3,0,0,1,.137,4.146,3.543,3.543,0,0,1-1.494,2.555A2.59,2.59,0,0,1,21.677,28.456Zm-9.58-10.2a27.949,27.949,0,0,0,5.492,6.929c2.249,1.935,4.033,2.351,4.8,1.9.8-.465,1.39-2.363.782-5.434A27.212,27.212,0,0,0,19.9,13.74,27.6,27.6,0,0,0,14.755,7.1c-2.424-2.152-4.39-2.633-5.191-2.169a2.327,2.327,0,0,0-.855,1.662,10.194,10.194,0,0,0,.153,3.565,27.465,27.465,0,0,0,3.236,8.1Z"
      fill="#00d8ff"
      stroke="#00d8ff"
      strokeWidth=".25"
    />
  </svg>
)))

export const VueSVG: AnyExoticRefComponent<SVGSVGElement> = React.forwardRef(((_props: any, ref: React.RefObject<SVGSVGElement> | ((instance: SVGSVGElement) => void) | null): JSX.Element => (
  <svg viewBox="0 0 32 32" width="128" height="128" ref={ref}>
    <path
      d="M24.4,3.925H30L16,28.075,2,3.925H12.71L16,9.525l3.22-5.6Z"
      fill="#41b883"
      stroke="#41b883"
      strokeWidth=".25"
    />
    <path
      d="M2,3.925l14,24.15L30,3.925H24.4L16,18.415,7.53,3.925Z"
      fill="#41b883"
      stroke="#41b883"
      strokeWidth=".25"
    />
    <path
      d="M7.53,3.925,16,18.485l8.4-14.56H19.22L16,9.525l-3.29-5.6Z"
      fill="#35495e"
      stroke="#35495e"
      strokeWidth=".25"
    />
  </svg>
)))

export const SvelteSVG: AnyExoticRefComponent<SVGSVGElement> = React.forwardRef(((_props: any, ref: React.RefObject<SVGSVGElement> | ((instance: SVGSVGElement) => void) | null): JSX.Element => (
  <svg viewBox="0 0 32 32" width="128" height="128" ref={ref}>
    <path d="M26.47,5.7A8.973,8.973,0,0,0,14.677,3.246L7.96,7.4a7.461,7.461,0,0,0-3.481,5.009,7.686,7.686,0,0,0,.8,5.058,7.358,7.358,0,0,0-1.151,2.8,7.789,7.789,0,0,0,1.4,6.028,8.977,8.977,0,0,0,11.794,2.458L24.04,24.6a7.468,7.468,0,0,0,3.481-5.009,7.673,7.673,0,0,0-.8-5.062,7.348,7.348,0,0,0,1.152-2.8A7.785,7.785,0,0,0,26.47,5.7"
      fill="#ff3e00"
      stroke="#ff3e00"
      strokeWidth=".25"
    />
    <path d="M14.022,26.64A5.413,5.413,0,0,1,8.3,24.581a4.678,4.678,0,0,1-.848-3.625,4.307,4.307,0,0,1,.159-.61l.127-.375.344.238a8.76,8.76,0,0,0,2.628,1.274l.245.073-.025.237a1.441,1.441,0,0,0,.271.968,1.63,1.63,0,0,0,1.743.636,1.512,1.512,0,0,0,.411-.175l6.7-4.154a1.366,1.366,0,0,0,.633-.909,1.407,1.407,0,0,0-.244-1.091,1.634,1.634,0,0,0-1.726-.622,1.509,1.509,0,0,0-.413.176l-2.572,1.584a4.934,4.934,0,0,1-1.364.582,5.415,5.415,0,0,1-5.727-2.06A4.678,4.678,0,0,1,7.811,13.1,4.507,4.507,0,0,1,9.9,10.09l6.708-4.154a4.932,4.932,0,0,1,1.364-.581A5.413,5.413,0,0,1,23.7,7.414a4.679,4.679,0,0,1,.848,3.625,4.272,4.272,0,0,1-.159.61l-.127.375-.344-.237a8.713,8.713,0,0,0-2.628-1.274l-.245-.074.025-.237a1.438,1.438,0,0,0-.272-.968,1.629,1.629,0,0,0-1.725-.622,1.484,1.484,0,0,0-.411.176l-6.722,4.14a1.353,1.353,0,0,0-.631.908,1.394,1.394,0,0,0,.244,1.092,1.634,1.634,0,0,0,1.726.621,1.538,1.538,0,0,0,.413-.175l2.562-1.585a4.9,4.9,0,0,1,1.364-.581,5.417,5.417,0,0,1,5.728,2.059,4.681,4.681,0,0,1,.843,3.625A4.5,4.5,0,0,1,22.1,21.905l-6.707,4.154a4.9,4.9,0,0,1-1.364.581"
      fill="#fff"
      stroke="#ff3e00"
      strokeWidth=".25"
    />
  </svg>
)))

export const CSSSVG: AnyExoticRefComponent<SVGSVGElement> = React.forwardRef(((_props: any, ref: React.RefObject<SVGSVGElement> | ((instance: SVGSVGElement) => void) | null): JSX.Element => (
  <svg
    viewBox="0 0 32 32"
    width="128" height="128" ref={ref}
  >
    <polygon
      points="5.902 27.201 3.656 2 28.344 2 26.095 27.197 15.985 30 5.902 27.201"
      fill="#1572b6"
      stroke="#1572b6"
      strokeWidth=".25"
    />
    <polygon
      points="16 27.858 24.17 25.593 26.092 4.061 16 4.061 16 27.858"
      fill="#33a9dc"
      stroke="#33a9dc"
      strokeWidth=".25"
    />
    <polygon
      points="16 13.191 20.09 13.191 20.372 10.026 16 10.026 16 6.935 16.011 6.935 23.75 6.935 23.676 7.764 22.917 16.282 16 16.282 16 13.191"
      fill="#fff"
      stroke="#fff"
      strokeWidth=".25"
    />
    <polygon
      points="16.019 21.218 16.005 21.222 12.563 20.292 12.343 17.827 10.67 17.827 9.24 17.827 9.673 22.68 16.004 24.438 16.019 24.434 16.019 21.218"
      fill="#ebebeb"
      stroke="#ebebeb"
      strokeWidth=".25"
    />
    <polygon
      points="19.827 16.151 19.455 20.29 16.008 21.22 16.008 24.436 22.344 22.68 22.391 22.158 22.928 16.151 19.827 16.151"
      fill="#fff"
      stroke="#fff"
      strokeWidth=".25"
    />
    <polygon
      points="16.011 6.935 16.011 8.855 16.011 10.018 16.011 10.026 8.555 10.026 8.555 10.026 8.545 10.026 8.483 9.331 8.342 7.764 8.268 6.935 16.011 6.935"
      fill="#ebebeb"
      stroke="#ebebeb"
      strokeWidth=".25"
    />
    <polygon
      points="16 13.191 16 15.111 16 16.274 16 16.282 12.611 16.282 12.611 16.282 12.601 16.282 12.539 15.587 12.399 14.02 12.325 13.191 16 13.191"
      fill="#ebebeb"
      stroke="#ebebeb"
      strokeWidth=".25"
    />
  </svg >
)))

export const StylusSVG: AnyExoticRefComponent<SVGSVGElement> = React.forwardRef(((_props: any, ref: React.RefObject<SVGSVGElement> | ((instance: SVGSVGElement) => void) | null): JSX.Element => (
  <svg
    viewBox="0 0 32 32"
    width="128" height="128" ref={ref}
  >
    <path
      d="M6.315,20.776c.726-.869.812-1.766.249-3.432-.356-1.054-.947-1.865-.513-2.521.463-.7,1.445-.021.627.911l.164.114c.983.114,1.467-1.232.733-1.616-1.937-1.011-3.631.933-2.884,3.183.32.954.769,1.965.406,2.77A1.794,1.794,0,0,1,3.773,21.3c-.847.043-.285-1.9.691-2.385.085-.043.206-.1.093-.242a2.134,2.134,0,0,0-2.314,1.2C1.061,22.121,4.485,22.954,6.315,20.776Z"
      fill="#c2c2c2"
      stroke="#c2c2c2"
      strokeWidth=".25"
    />
    <path
      d="M27.462,14.14c.278.684.7,1.36.449,1.958-.206.513-.477.726-.776.776-.42.071-.306-1.246.413-1.638.064-.036.157-.206.071-.306a1.636,1.636,0,0,0-1.7.961c-.8,1.687,1.823,2.15,3.112.491.513-.662.534-1.317.043-2.506-.313-.755-.79-1.317-.491-1.816.32-.527,1.089-.071.513.634l.128.071c.748.043,1.054-.954.484-1.2A1.834,1.834,0,0,0,27.462,14.14Z"
      fill="#c2c2c2"
      stroke="#c2c2c2"
      strokeWidth=".25"
    />
    <path
      d="M17.95,12.338c-.52-.413-1.979.278-2.392,1.3a16.213,16.213,0,0,1-2.043,4.037c-.8.876-.876.2-.8-.306A15.327,15.327,0,0,1,14.7,12.652c-.235-.349-1.773-.3-2.841,1.36-.4.627-1.31,2.713-2.321,4.358-.221.356-.5.107-.285-.726a39.8,39.8,0,0,1,1.873-5.682,39.072,39.072,0,0,1,6.921-.819c.263-.071.441-.306,0-.32a46.778,46.778,0,0,0-6.593.441,4.923,4.923,0,0,1,1.445-1.944,1.787,1.787,0,0,0-2.264.719,12.287,12.287,0,0,0-.819,1.453,24.194,24.194,0,0,0-3.98.862c-.783.3-.7,1.246-.221,1.068a31.631,31.631,0,0,1,3.816-1.1,30.292,30.292,0,0,0-1.865,5.682c-.441,2.492,1.1,2.478,1.858,1.5.819-1.075,2.528-4.856,2.791-5.255.078-.135.185-.064.128.057-1.908,3.809-1.744,5.283-.2,4.956a4.929,4.929,0,0,0,2.214-1.965c.064-.15.2-.135.171-.071-1.21,3.14-2.748,5.682-3.781,6.479-.94.719-1.638-.84,1.687-3.076.491-.335.263-.79-.292-.634h0a29.793,29.793,0,0,0-8.786,3.325c-.164.114-.313.206-.306.441.007.135.242.085.356.014A22.771,22.771,0,0,1,11.1,20.9a.158.158,0,0,0,.114.007c.121-.028.114.036.036.085-.178.1-.356.192-.4.206-1.766.691-2.834,2.214-2.456,2.99.32.669,2.051.427,2.869-.014,2.008-1.089,3.468-3.225,4.464-6.173C16.6,15.386,17.693,12.424,17.95,12.338Z"
      fill="#c2c2c2"
      stroke="#c2c2c2"
      strokeWidth=".25"
    />
    <path
      d="M29.62,18.369a48.145,48.145,0,0,0-13.429.968c-.926.242-.669.733-.2.641.007,0,.206-.05.214-.05a42.279,42.279,0,0,1,12.375-.242C29.014,19.765,30.317,18.462,29.62,18.369Z"
      fill="#c2c2c2"
      stroke="#c2c2c2"
      strokeWidth=".25"
    />
    <path
      d="M18.861,17.985c.911-.456,2.264-3.275,3.154-4.82.064-.114.178-.021.114.057-2.25,3.873-1.3,4.322-.406,4.265,1.189-.071,2.286-1.78,2.528-2.165.1-.15.157-.028.1.078-.057.178-.263.491-.456.919-.271.605.014.84.249.947.37.178,1.381.064,1.538-.555-1.011-.021,1.41-4.792,1.659-5.084a1.718,1.718,0,0,0-2.2.975c-1.018,2.015-1.873,3.638-2.407,3.667-1.04.057,1.2-4.493,1.559-4.635-.221-.32-1.638-.185-2.428,1.04-.285.441-2.022,3.517-2.449,4.023-.755.9-.812.128-.6-.769a10.569,10.569,0,0,1,.349-1.132,5.324,5.324,0,0,1,1.36-1.844c2.2-2.442,3.46-4.422,2.962-5.2-.441-.691-1.915-.384-2.862,1.04-1.744,2.613-3.354,6.195-3.56,7.832S18.056,18.383,18.861,17.985Zm.926-4.792c.078-.178.128-.228.263-.527h0A24.934,24.934,0,0,1,22.492,8.28c.42-.441,1.011.157-.057,1.794a16.462,16.462,0,0,1-2.115,2.642v.007c-.2.221-.377.406-.456.513C19.808,13.307,19.744,13.292,19.787,13.193Z"
      fill="#c2c2c2"
      stroke="#c2c2c2"
      strokeWidth=".25"
    />
  </svg>
)))

export const SassSVG: AnyExoticRefComponent<SVGSVGElement> = React.forwardRef(((_props: any, ref: React.RefObject<SVGSVGElement> | ((instance: SVGSVGElement) => void) | null): JSX.Element => (
  <svg
    viewBox="0 0 32 32"
    width="128" height="128" ref={ref}
  >
    <path
      d="M26.11,17.572a5.8,5.8,0,0,0-2.537.588,5.345,5.345,0,0,1-.568-1.314,3.53,3.53,0,0,1-.051-1.1,9.811,9.811,0,0,1,.332-1.192c-.005-.051-.061-.292-.624-.3s-1.048.107-1.1.256a6.171,6.171,0,0,0-.235.834,19.686,19.686,0,0,1-1.713,3.294,3.186,3.186,0,0,1-.44-2.066,9.811,9.811,0,0,1,.332-1.192c-.005-.051-.061-.292-.624-.3s-1.048.107-1.1.256-.118.5-.235.834-1.483,3.386-1.841,4.173c-.184.4-.343.726-.455.946h0a.233.233,0,0,1-.02.041c-.1.189-.153.292-.153.292v.005c-.077.138-.159.266-.2.266a1.711,1.711,0,0,1,.01-.869c.2-1.059.69-2.705.685-2.762,0-.031.092-.317-.317-.465a.508.508,0,0,0-.578.1c-.036,0-.061.087-.061.087s.445-1.851-.849-1.851a3.855,3.855,0,0,0-2.475,1.683c-.348.189-1.089.593-1.882,1.028-.3.169-.614.338-.905.5-.02-.02-.041-.046-.061-.066C6.87,17.6,3.975,16.416,4.1,14.171c.046-.818.327-2.966,5.559-5.575,4.306-2.122,7.733-1.534,8.326-.23.849,1.862-1.836,5.319-6.285,5.82a3.351,3.351,0,0,1-2.813-.711c-.235-.256-.271-.271-.358-.22-.143.077-.051.307,0,.44a2.626,2.626,0,0,0,1.606,1.263,8.55,8.55,0,0,0,5.217-.517c2.7-1.043,4.8-3.943,4.184-6.372-.619-2.465-4.71-3.278-8.582-1.9A19.5,19.5,0,0,0,4.359,9.952c-2.133,1.995-2.47,3.728-2.332,4.455.5,2.578,4.051,4.255,5.472,5.5-.072.041-.138.077-.194.107-.711.353-3.421,1.77-4.1,3.268-.767,1.7.123,2.915.711,3.079a4.374,4.374,0,0,0,4.71-1.908A4.725,4.725,0,0,0,9.049,20.1a.107.107,0,0,0-.02-.031l.557-.327c.363-.215.721-.414,1.028-.578a6.74,6.74,0,0,0-.363,1.862,3.886,3.886,0,0,0,.834,2.7.921.921,0,0,0,.675.22c.6,0,.875-.5,1.176-1.094.368-.726.7-1.57.7-1.57s-.414,2.281.711,2.281c.409,0,.823-.532,1.008-.8v.005s.01-.015.031-.051l.066-.107v-.01c.164-.286.532-.936,1.079-2.015.706-1.391,1.386-3.13,1.386-3.13a8.888,8.888,0,0,0,.271,1.13,10.643,10.643,0,0,0,.583,1.309c-.164.23-.266.358-.266.358l.005.005c-.133.174-.276.363-.435.547a16.3,16.3,0,0,0-1.314,1.647.447.447,0,0,0,.123.6,1.116,1.116,0,0,0,.685.113,3.147,3.147,0,0,0,1.028-.235,3.45,3.45,0,0,0,.885-.465,1.98,1.98,0,0,0,.849-1.744,3.521,3.521,0,0,0-.322-1.233c.051-.072.1-.143.148-.215a23.428,23.428,0,0,0,1.534-2.649,8.888,8.888,0,0,0,.271,1.13,7.57,7.57,0,0,0,.5,1.125A4.861,4.861,0,0,0,20.965,20.8c-.322.931-.072,1.35.4,1.447a1.425,1.425,0,0,0,.747-.153,3.4,3.4,0,0,0,.946-.486,2.126,2.126,0,0,0,1.043-1.729,3.268,3.268,0,0,0-.235-1.023,5.356,5.356,0,0,1,2.716-.312c2.434.286,2.915,1.805,2.823,2.445a1.618,1.618,0,0,1-.772,1.094c-.169.107-.225.143-.21.22.02.113.1.107.245.087A1.9,1.9,0,0,0,30,20.7c.077-1.5-1.355-3.145-3.887-3.13ZM7.33,23.9c-.808.88-1.933,1.212-2.419.931-.522-.3-.317-1.6.675-2.532a12.884,12.884,0,0,1,1.9-1.417c.118-.072.292-.174.5-.3l.056-.031h0l.123-.077A3.493,3.493,0,0,1,7.33,23.9Zm5.881-4c-.281.685-.869,2.44-1.227,2.342-.307-.082-.5-1.412-.061-2.726a6.193,6.193,0,0,1,.956-1.754c.44-.491.926-.655,1.043-.455a9.062,9.062,0,0,1-.711,2.593Zm4.853,2.322c-.118.061-.23.1-.281.072-.036-.02.051-.1.051-.1s.609-.655.849-.951c.138-.174.3-.378.476-.609V20.7c0,.782-.757,1.309-1.094,1.524Zm3.744-.854c-.087-.061-.072-.266.22-.905a3.408,3.408,0,0,1,.834-1.074,1.448,1.448,0,0,1,.082.471,1.547,1.547,0,0,1-1.135,1.509Z"
      fill="#cd6799"
      stroke="#cd6799"
      strokeWidth=".25"
    />
  </svg>
)))

export const GatsbySVG: AnyExoticRefComponent<SVGSVGElement> = React.forwardRef(((_props: any, ref: React.RefObject<SVGSVGElement> | ((instance: SVGSVGElement) => void) | null): JSX.Element => (
  <svg
    viewBox="0 0 32 32"
    width="128" height="128" ref={ref}
  >
    <title>file_type_gatsby</title>
    <path
      d="M27,16H20v2h4.8A9.066,9.066,0,0,1,19,24.5L7.5,13A9,9,0,0,1,16,7a9.232,9.232,0,0,1,7.4,3.8l1.5-1.3A10.993,10.993,0,0,0,5.3,13.6L18.5,26.8A11.251,11.251,0,0,0,27,16ZM5,16.1a10.682,10.682,0,0,0,3.2,7.6,10.855,10.855,0,0,0,7.6,3.2Z"
      fill="#fff"
      stroke="#fff"
      strokeWidth=".25"
    />
    <path
      d="M16,2A14,14,0,1,0,30,16,14.041,14.041,0,0,0,16,2ZM8.2,23.8A10.855,10.855,0,0,1,5,16.2L15.9,27A11.351,11.351,0,0,1,8.2,23.8Zm10.2,2.9L5.3,13.6A10.993,10.993,0,0,1,24.9,9.5l-1.5,1.3A9.232,9.232,0,0,0,16,7a9.1,9.1,0,0,0-8.5,6L19,24.5A9.066,9.066,0,0,0,24.8,18H20V16h7A11.031,11.031,0,0,1,18.4,26.7Z"
      fill="#639"
      stroke="#639"
      strokeWidth=".25"
    />
  </svg>
)))

export const GitSVG: AnyExoticRefComponent<SVGSVGElement> = React.forwardRef(((_props: any, ref: React.RefObject<SVGSVGElement> | ((instance: SVGSVGElement) => void) | null): JSX.Element => (
  <svg
    viewBox="0 0 32 32"
    width="128" height="128" ref={ref}
  >
    <path d="M29.472,14.753,17.247,2.528a1.8,1.8,0,0,0-2.55,0L12.158,5.067l3.22,3.22a2.141,2.141,0,0,1,2.712,2.73l3.1,3.1a2.143,2.143,0,1,1-1.285,1.21l-2.895-2.895v7.617a2.141,2.141,0,1,1-1.764-.062V12.3a2.146,2.146,0,0,1-1.165-2.814L10.911,6.314,2.528,14.7a1.8,1.8,0,0,0,0,2.551L14.753,29.472a1.8,1.8,0,0,0,2.55,0L29.472,17.3a1.8,1.8,0,0,0,0-2.551"
      fill="#dd4c35"
      stroke="#dd4c35"
      strokeWidth=".25"
    />
    <path d="M12.158,5.067l3.22,3.22a2.141,2.141,0,0,1,2.712,2.73l3.1,3.1a2.143,2.143,0,1,1-1.285,1.21l-2.895-2.895v7.617a2.141,2.141,0,1,1-1.764-.062V12.3a2.146,2.146,0,0,1-1.165-2.814L10.911,6.314"
      fill="#fff"
      stroke="#fff"
      strokeWidth=".25"
    />
  </svg>
)))

export const JestSVG: AnyExoticRefComponent<SVGSVGElement> = React.forwardRef(((_props: any, ref: React.RefObject<SVGSVGElement> | ((instance: SVGSVGElement) => void) | null): JSX.Element => (
  <svg
    viewBox="0 0 32 32"
    width="128" height="128" ref={ref}
  >
    <path d="M27.089,15.786a2.606,2.606,0,0,0-2.606-2.606,2.633,2.633,0,0,0-.274.014l3.58-10.558H12.576l3.575,10.549c-.052,0-.105-.005-.158-.005a2.606,2.606,0,0,0-.791,5.09,11.383,11.383,0,0,1-2.049,2.579A10.436,10.436,0,0,1,9.5,23.116a3.323,3.323,0,0,1-1.665-4.23c.077-.18.155-.362.229-.544a2.607,2.607,0,1,0-2.09-.4,20.134,20.134,0,0,0-1.889,4.787c-.354,2.135,0,4.4,1.845,5.681,4.3,2.981,8.969-1.848,13.891-3.061,1.784-.44,3.742-.369,5.313-1.279a4.45,4.45,0,0,0,2.179-3.088,4.639,4.639,0,0,0-.831-3.522,2.6,2.6,0,0,0,.606-1.671ZM18.6,15.8v-.009a2.6,2.6,0,0,0-1.256-2.23L20.188,7.8l2.85,5.814a2.6,2.6,0,0,0-1.161,2.169c0,.019,0,.038,0,.057L18.6,15.8Z"
      fill="#c63d14"
      stroke="#c63d14"
      strokeWidth=".25"
    />
    <path d="M27.726,15.786A3.247,3.247,0,0,0,25.083,12.6L28.677,2H11.689l3.6,10.621a3.241,3.241,0,0,0-1,5.918,10.865,10.865,0,0,1-1.568,1.846,9.827,9.827,0,0,1-3.16,2.03,2.656,2.656,0,0,1-1.134-3.28l.04-.093.074-.175a3.245,3.245,0,1,0-3.34-.754c-.108.231-.222.465-.339.706a14,14,0,0,0-1.4,3.8c-.465,2.8.285,5.043,2.111,6.308A5.751,5.751,0,0,0,8.934,30c2.132,0,4.289-1.071,6.4-2.119a23.672,23.672,0,0,1,4.642-1.918c.577-.142,1.171-.228,1.8-.319a9.977,9.977,0,0,0,3.68-1.028,5.09,5.09,0,0,0,2.487-3.53,5.255,5.255,0,0,0-.691-3.613,3.223,3.223,0,0,0,.475-1.688Zm-1.273,0a1.97,1.97,0,1,1-1.97-1.97,1.972,1.972,0,0,1,1.97,1.97ZM26.9,3.273l-3.174,9.36a3.2,3.2,0,0,0-.4.125l-3.135-6.4L17.046,12.72a3.2,3.2,0,0,0-.419-.114L13.464,3.273ZM15.993,13.816a1.97,1.97,0,1,1-1.97,1.97,1.972,1.972,0,0,1,1.97-1.97Zm-8.522.019A1.97,1.97,0,1,1,5.5,15.8a1.972,1.972,0,0,1,1.97-1.97Zm19.217,7.034a3.82,3.82,0,0,1-1.871,2.646,8.9,8.9,0,0,1-3.223.869c-.632.091-1.285.185-1.924.343a24.587,24.587,0,0,0-4.9,2.013c-3.164,1.571-5.9,2.928-8.472,1.143-1.887-1.308-1.8-3.728-1.58-5.054A12.82,12.82,0,0,1,6,19.376c.092-.189.182-.375.27-.559a3.215,3.215,0,0,0,.838.21,3.965,3.965,0,0,0,2.1,4.655l.245.126.259-.092a11.029,11.029,0,0,0,3.875-2.4,12.078,12.078,0,0,0,1.918-2.319,3.241,3.241,0,0,0,2.64-5.627l2.038-4.121,2.059,4.2a3.24,3.24,0,0,0,4.088,5,3.957,3.957,0,0,1,.355,2.422Z"
      fill="#fff"
      stroke="#fff"
      strokeWidth=".25"
    />
  </svg>
)))

export const GraphQLSVG: AnyExoticRefComponent<SVGSVGElement> = React.forwardRef(((_props: any, ref: React.RefObject<SVGSVGElement> | ((instance: SVGSVGElement) => void) | null): JSX.Element => (
  <svg
    viewBox="0 0 32 32"
    width="128" height="128" ref={ref}
  >
    <rect x="-0.43" y="12.534" width="22.901" height="1.187" transform="translate(-5.858 16.107) rotate(-59.999)"
      fill="#e10098"
      stroke="#e10098"
      strokeWidth=".25"
    />
    <rect x="4.545" y="21.162" width="22.902" height="1.187"
      fill="#e10098"
      stroke="#e10098"
      strokeWidth=".25"
    />
    <rect x="10.43" y="18.008" width="1.187" height="13.227" transform="translate(-15.812 21.857) rotate(-59.999)"
      fill="#e10098"
      stroke="#e10098"
      strokeWidth=".25"
    />
    <rect x="20.381" y="0.771" width="1.187" height="13.227" transform="translate(4.092 21.856) rotate(-59.999)"
      fill="#e10098"
      stroke="#e10098"
      strokeWidth=".25"
    />
    <rect x="4.412" y="6.787" width="13.227" height="1.187" transform="translate(-2.213 6.502) rotate(-30.001)"
      fill="#e10098"
      stroke="#e10098"
      strokeWidth=".25"
    />
    <rect x="20.389" y="1.677" width="1.187" height="22.901" transform="translate(-3.753 12.25) rotate(-30.001)"
      fill="#e10098"
      stroke="#e10098"
      strokeWidth=".25"
    />
    <rect x="5.454" y="9.386" width="1.187" height="13.228"
      fill="#e10098"
      stroke="#e10098"
      strokeWidth=".25"
    />
    <rect x="25.36" y="9.386" width="1.187" height="13.228"
      fill="#e10098"
      stroke="#e10098"
      strokeWidth=".25"
    />
    <rect x="15.222" y="24.097" width="11.504" height="1.037" transform="translate(-9.498 13.785) rotate(-30.001)"
      fill="#e10098"
      stroke="#e10098"
      strokeWidth=".25"
    />
    <path d="M28.12,23a2.5,2.5,0,1,1-.915-3.411A2.5,2.5,0,0,1,28.12,23"
      fill="#e10098"
      stroke="#e10098"
      strokeWidth=".25"
    />
    <path d="M8.2,11.5a2.5,2.5,0,1,1-.915-3.411A2.5,2.5,0,0,1,8.2,11.5"
      fill="#e10098"
      stroke="#e10098"
      strokeWidth=".25"
    />
    <path d="M3.88,23a2.5,2.5,0,1,1,3.411.915A2.5,2.5,0,0,1,3.88,23"
      fill="#e10098"
      stroke="#e10098"
      strokeWidth=".25"
    />
    <path d="M23.8,11.5a2.5,2.5,0,1,1,3.411.915A2.5,2.5,0,0,1,23.8,11.5"
      fill="#e10098"
      stroke="#e10098"
      strokeWidth=".25"
    />
    <path d="M16,30a2.5,2.5,0,1,1,2.5-2.5A2.493,2.493,0,0,1,16,30"
      fill="#e10098"
      stroke="#e10098"
      strokeWidth=".25"
    />
    <path d="M16,6.991a2.5,2.5,0,1,1,2.5-2.5,2.493,2.493,0,0,1-2.5,2.5"
      fill="#e10098"
      stroke="#e10098"
      strokeWidth=".25"
    />
  </svg>
)))

export const MeteorSVG: AnyExoticRefComponent<SVGSVGElement> = React.forwardRef(((_props: any, ref: React.RefObject<SVGSVGElement> | ((instance: SVGSVGElement) => void) | null): JSX.Element => (
  <svg
    viewBox="0 0 32 32"
    width="128" height="128" ref={ref}
  >
    <path d="M2,2.273l23.979,25.4a1.119,1.119,0,0,0,1.442-.1,1.064,1.064,0,0,0,.144-1.344L2,2.273Z"
      fill="#df4f4f"
      stroke="#df4f4f"
      strokeWidth=".25"
    />
    <path d="M9.593,4.674,27.853,24.36a1.119,1.119,0,0,0,1.442-.1,1.064,1.064,0,0,0,.144-1.344L9.593,4.674Z"
      fill="#df4f4f"
      stroke="#df4f4f"
      strokeWidth=".25"
    />
    <path d="M4.259,9.811,22.519,29.5a1.119,1.119,0,0,0,1.442-.1,1.064,1.064,0,0,0,.144-1.344L4.259,9.811Z"
      fill="#df4f4f"
      stroke="#df4f4f"
      strokeWidth=".25"
    />
    <path d="M16.011,6.738,28.769,20.492a.782.782,0,0,0,1.007-.067.743.743,0,0,0,.1-.939L16.011,6.738Z"
      fill="#df4f4f"
      stroke="#df4f4f"
      strokeWidth=".25"
    />
    <path d="M6.016,15.765,18.773,29.518a.782.782,0,0,0,1.007-.067.743.743,0,0,0,.1-.939L6.016,15.765Z"
      fill="#df4f4f"
      stroke="#df4f4f"
      strokeWidth=".25"
    />
    <path d="M22.567,9.715l5.782,6.252a.4.4,0,0,0,.5-.031.335.335,0,0,0,.05-.44l-6.33-5.781Z"
      fill="#df4f4f"
      stroke="#df4f4f"
      strokeWidth=".25"
    />
    <path d="M9.208,22.1l5.782,6.252a.4.4,0,0,0,.5-.031.335.335,0,0,0,.05-.44L9.208,22.1Z"
      fill="#df4f4f"
      stroke="#df4f4f"
      strokeWidth=".25"
    />
  </svg>
)))

export const NodeSVG: AnyExoticRefComponent<SVGSVGElement> = React.forwardRef(((_props: any, ref: React.RefObject<SVGSVGElement> | ((instance: SVGSVGElement) => void) | null): JSX.Element => (
  <svg
    viewBox="0 0 32 32"
    width="128" height="128" ref={ref}
  >
    <path d="M16,30a2.151,2.151,0,0,1-1.076-.288L11.5,27.685c-.511-.286-.262-.387-.093-.446a6.828,6.828,0,0,0,1.549-.7.263.263,0,0,1,.255.019l2.631,1.563a.34.34,0,0,0,.318,0l10.26-5.922a.323.323,0,0,0,.157-.278V10.075a.331.331,0,0,0-.159-.283L16.158,3.875a.323.323,0,0,0-.317,0L5.587,9.794a.33.33,0,0,0-.162.281V21.916a.315.315,0,0,0,.161.274L8.4,23.814c1.525.762,2.459-.136,2.459-1.038V11.085a.3.3,0,0,1,.3-.3h1.3a.3.3,0,0,1,.3.3V22.777c0,2.035-1.108,3.2-3.038,3.2a4.389,4.389,0,0,1-2.363-.642L4.661,23.788a2.166,2.166,0,0,1-1.076-1.872V10.075A2.162,2.162,0,0,1,4.661,8.2L14.922,2.276a2.246,2.246,0,0,1,2.156,0L27.338,8.2a2.165,2.165,0,0,1,1.077,1.87V21.916a2.171,2.171,0,0,1-1.077,1.872l-10.26,5.924A2.152,2.152,0,0,1,16,30Z"
      fill="#83cd29"
      stroke="#83cd29"
      strokeWidth=".25"
    />
    <path d="M14.054,17.953a.3.3,0,0,1,.3-.3h1.327a.3.3,0,0,1,.295.251c.2,1.351.8,2.032,3.513,2.032,2.161,0,3.082-.489,3.082-1.636,0-.661-.261-1.152-3.62-1.481-2.808-.278-4.544-.9-4.544-3.144,0-2.07,1.745-3.305,4.67-3.305,3.287,0,4.914,1.141,5.12,3.589a.3.3,0,0,1-.295.323H22.566a.3.3,0,0,1-.288-.232c-.319-1.421-1.1-1.875-3.2-1.875-2.36,0-2.634.822-2.634,1.438,0,.746.324.964,3.51,1.385,3.153.417,4.651,1.007,4.651,3.223,0,2.236-1.864,3.516-5.115,3.516C14.995,21.743,14.054,19.682,14.054,17.953Z"
      fill="#83cd29"
      stroke="#83cd29"
      strokeWidth=".25"
    />
  </svg>
)))

export const MongoSVG: AnyExoticRefComponent<SVGSVGElement> = React.forwardRef(((_props: any, ref: React.RefObject<SVGSVGElement> | ((instance: SVGSVGElement) => void) | null): JSX.Element => (
  <svg
    viewBox="0 0 32 32"
    width="128" height="128" ref={ref}
  >
    <path d="M16.62,30l-.751-.249s.1-3.8-1.275-4.067c-.9-1.048.133-44.741,3.423-.149a2.712,2.712,0,0,0-1.333,1.523A14.1,14.1,0,0,0,16.62,30Z"
      fill="#999875"
      stroke="#999875"
      strokeWidth=".25"
    />
    <path d="M17.026,26.329a13.223,13.223,0,0,0,5-13.225C20.556,6.619,17.075,4.487,16.7,3.673a9.792,9.792,0,0,1-.825-1.6l.277,18.069S15.578,25.664,17.026,26.329Z"
      fill="#48a547"
      stroke="#48a547"
      strokeWidth=".25"
    />
    <path d="M15.487,26.569S9.366,22.4,9.72,15.025A15.54,15.54,0,0,1,15.239,3.377,1.725,1.725,0,0,0,15.846,2c.381.82.319,12.243.359,13.579C16.36,20.776,15.916,25.588,15.487,26.569Z"
      fill="#41a247"
      stroke="#41a247"
      strokeWidth=".25"
    />
  </svg>
)))

export const HTMLSVG: AnyExoticRefComponent<SVGSVGElement> = React.forwardRef(((_props: any, ref: React.RefObject<SVGSVGElement> | ((instance: SVGSVGElement) => void) | null): JSX.Element => (
  <svg
    viewBox="0 0 32 32"
    width="128" height="128" ref={ref}
  >
    <polygon points="5.902 27.201 3.655 2 28.345 2 26.095 27.197 15.985 30 5.902 27.201"
      fill="#e44f26"
      stroke="#e44f26"
      strokeWidth=".25"
    />
    <polygon points="16 27.858 24.17 25.593 26.092 4.061 16 4.061 16 27.858"
      fill="#f1662a"
      stroke="#f1662a"
      strokeWidth=".25"
    />
    <polygon points="16 13.407 11.91 13.407 11.628 10.242 16 10.242 16 7.151 15.989 7.151 8.25 7.151 8.324 7.981 9.083 16.498 16 16.498 16 13.407"
      fill="#ebebeb"
      stroke="#ebebeb"
      strokeWidth=".25"
    />
    <polygon points="16 21.434 15.986 21.438 12.544 20.509 12.324 18.044 10.651 18.044 9.221 18.044 9.654 22.896 15.986 24.654 16 24.65 16 21.434"
      fill="#ebebeb"
      stroke="#ebebeb"
      strokeWidth=".25"
    />
    <polygon points="15.989 13.407 15.989 16.498 19.795 16.498 19.437 20.507 15.989 21.437 15.989 24.653 22.326 22.896 22.372 22.374 23.098 14.237 23.174 13.407 22.341 13.407 15.989 13.407"
      fill="#fff"
      stroke="#fff"
      strokeWidth=".25"
    />
    <polygon points="15.989 7.151 15.989 9.071 15.989 10.235 15.989 10.242 23.445 10.242 23.445 10.242 23.455 10.242 23.517 9.548 23.658 7.981 23.732 7.151 15.989 7.151"
      fill="#fff"
      stroke="#fff"
      strokeWidth=".25"
    />
  </svg>
)))

export const JSSVG: AnyExoticRefComponent<SVGSVGElement> = React.forwardRef(((_props: any, ref: React.RefObject<SVGSVGElement> | ((instance: SVGSVGElement) => void) | null): JSX.Element => (
  <svg
    viewBox="0 0 32 32"
    width="128" height="128" ref={ref}
  >
    <rect x="2" y="2" width="28" height="28"
      fill="#f5de19"
      stroke="#f5de19"
      strokeWidth=".25"
    />
    <path d="M20.809,23.875a2.866,2.866,0,0,0,2.6,1.6c1.09,0,1.787-.545,1.787-1.3,0-.9-.716-1.222-1.916-1.747l-.658-.282c-1.9-.809-3.16-1.822-3.16-3.964,0-1.973,1.5-3.476,3.853-3.476a3.889,3.889,0,0,1,3.742,2.107L25,18.128A1.789,1.789,0,0,0,23.311,17a1.145,1.145,0,0,0-1.259,1.128c0,.789.489,1.109,1.618,1.6l.658.282c2.236.959,3.5,1.936,3.5,4.133,0,2.369-1.861,3.667-4.36,3.667a5.055,5.055,0,0,1-4.795-2.691Zm-9.295.228c.413.733.789,1.353,1.693,1.353.864,0,1.41-.338,1.41-1.653V14.856h2.631v8.982c0,2.724-1.6,3.964-3.929,3.964a4.085,4.085,0,0,1-3.947-2.4Z"
    />
  </svg>
)))

export const TSSVG: AnyExoticRefComponent<SVGSVGElement> = React.forwardRef(((_props: any, ref: React.RefObject<SVGSVGElement> | ((instance: SVGSVGElement) => void) | null): JSX.Element => (
  <svg
    viewBox="0 0 32 32"
    width="128" height="128" ref={ref}
  >
    <polygon points="2 16 2 30 16 30 30 30 30 16 30 2 16 2 2 2 2 16"
      fill="#007acc"
      stroke="#007acc"
      strokeWidth=".25"
    />
    <path d="M24.564,14.884a3.485,3.485,0,0,1,1.751,1.009,4.611,4.611,0,0,1,.671.9c.009.036-1.209.853-1.947,1.311-.027.018-.133-.1-.253-.276a1.587,1.587,0,0,0-1.316-.791c-.849-.058-1.4.387-1.391,1.129a1.027,1.027,0,0,0,.12.524c.187.387.533.618,1.622,1.089,2,.862,2.862,1.431,3.4,2.24a4.063,4.063,0,0,1,.324,3.413,3.753,3.753,0,0,1-3.1,2.218,8.584,8.584,0,0,1-2.133-.022,5.145,5.145,0,0,1-2.849-1.484,4.947,4.947,0,0,1-.729-1.08,2.092,2.092,0,0,1,.258-.164c.124-.071.6-.342,1.04-.6l.8-.467L21,24.08A3.759,3.759,0,0,0,22.067,25.1a2.6,2.6,0,0,0,2.724-.138,1.217,1.217,0,0,0,.156-1.551c-.218-.311-.662-.573-1.924-1.12a6.93,6.93,0,0,1-2.636-1.622,3.692,3.692,0,0,1-.769-1.4,5.606,5.606,0,0,1-.049-1.787,3.413,3.413,0,0,1,2.871-2.658A7.092,7.092,0,0,1,24.564,14.884Zm-6.573,1.169L18,17.2H14.356V27.556H11.778V17.2H8.133V16.076a11.018,11.018,0,0,1,.031-1.156c.013-.018,2.231-.027,4.92-.022l4.893.013Z"
      fill="#fff"
      stroke="#fff"
      strokeWidth=".25"
    />
  </svg>
)))