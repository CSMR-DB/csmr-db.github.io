...
import { AnyExoticRefComponent, IBioImagesEdge, IBioImagesMarkdownRemark } from '../types.interface';
...
const GamesDecor: AnyExoticRefComponent<any> = React.forwardRef((
  ...
  const { wallpapers }: { wallpapers: IBioImagesMarkdownRemark } = useStaticQuery(graphql`
    query {
  ...
  return targets !== null ? (
    <div ref={(el: HTMLDivElement): void => targets.set('sceneRoot', el)}>
      <Layer ref={(el: HTMLDivElement): void => targets.set('games', el)}>
        <div ref={(el: HTMLDivElement): void => {
          if (el !== null && el.childNodes !== null) {
            targets.set('gamesStaggered', el.childNodes)
          }
        }}>
          {
            wallpapers.edges.filter((edge: IBioImagesEdge): boolean => edge.node.relativePath.includes('games')).map((edge: IBioImagesEdge, i: number): JSX.Element =>
              <Absolute key={i}>
                <Filter>
      ...
      <Layer>
        <h1 ref={(el: HTMLDivElement): void => targets.set('myGames', el)}>I play my games</h1>
      </Layer>
    </div>
  ) : null
});

export function GamesIvePlayed({ position }: { position: string }): JSX.Element {
