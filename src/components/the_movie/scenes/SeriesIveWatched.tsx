...
import { ISceneProps, AnyExoticRefComponent, IBioImagesEdge, IBioImagesMarkdownRemark } from '../types.interface';
...
const ShowsGrid: StyledComponent<"div", any, {}, never> = styled.div`
  ...
  filter: sepia(.5) grayscale(.5) blur(1px) brightness(.5);
  position: absolute;
  width: 100%;
  height: 100%;
  top: 0;
...
const SeriesDecor: AnyExoticRefComponent<any> = React.forwardRef((
  ...
  const { posters }: { posters: IBioImagesMarkdownRemark } = useStaticQuery(graphql`
  query {
  ...
  return targets !== null ? (
    <div ref={(el: HTMLDivElement): void => targets.set('sceneRoot', el)}>
      <Layer ref={(el: HTMLDivElement): void => targets.set('series', el)}>
        <ShowsGrid ref={(el: HTMLDivElement): void => {
          if (el !== null && el.childNodes !== null) {
            targets.set('seriesStaggered', el.childNodes)
          }
        }}>
          {
            posters.edges.filter((edge: IBioImagesEdge): boolean => edge.node.relativePath.includes('series')).map((edge: IBioImagesEdge, i: number): JSX.Element =>
              <div key={i}>
      ...
      <Layer ref={(el: HTMLDivElement): void => targets.set('overlayText', el)}>
        <Blend mode={'lighten'}>
          <BoldH1>
            <SplitChars ref={(el: HTMLHeadingElement): void => targets.set('overlayTextSplit', [el])} wrapper={<span />}>...watch my shows...</SplitChars>
          </BoldH1>
        </Blend>
      </Layer>
    </div>
...
export function SeriesIveWatched({ position }: { position: string }): JSX.Element {
  ...
  return (
    ...
    >
      ...
      <Tween
        ...
        duration={.05}
        stagger={{ from: 'random', amount: .25, grid: [8, 3] }}
        // stagger={.025}
