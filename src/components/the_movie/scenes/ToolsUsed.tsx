...
const ColoredText: StyledComponent<"span", any, Partial<ITool>, never> = styled.span`
  color: ${(props: ITool): ColorProperty => props.color || 'black'};
`

const ColoredCircle: StyledComponent<"div", any, Partial<ITool>, never> = styled.div`
  width: 8rem;
  height: 8rem;
...
const TimelineTarget: AnyExoticRefComponent<any> = React.forwardRef((
  ...
  return (
    <div ref={(el: HTMLDivElement): void => targets.set('sceneRoot', el)}>
      <Layer ref={(el: HTMLHeadingElement): void => targets.set('frameworks', el)}>
        <Grid
          columns={4}
          rows={4}
        ...
        >
          {
            allSVGs.map((framework: ITool, i: number): JSX.Element => (
              <ColoredCircle
      ...
      <Layer
        ref={(el: HTMLDivElement): void => targets.set('intro', el)}
      >
        {/* <Flex direction="row">
          <h2>
        ...
        <WriteWithCursor
          ref={(el: HTMLHeadingElement): void => targets.set('introSplit', [el])}
...
export function ToolsUsed({ position }: { position: string }): JSX.Element {
  ...
  return (
    ...
    >
      ...
      <InOut
        positions={['fadeInRoot', 'frameworks']}
        target={'intro'}
      />
      <Write
        position={'introText'}
      ...
      <Tween
        ...
        stagger={{ from: 'random', amount: .5, grid: [4, 4] }}
        duration={1}
        ease={'elastic'}
        position={'frameworks'}
        target='codeIcons'
      />
      <InOut
        positions={['frameworks', 'outroText']}
        target={'frameworks'}
      />
      {
        allSVGs.map((framework: ITool, i: number): JSX.Element => (
