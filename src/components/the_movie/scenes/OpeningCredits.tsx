  ...
  return (
    <div ref={(el: HTMLDivElement): void => targets.set('sceneRoot', el)}>
      <Layer ref={(el: HTMLDivElement): void => targets.set('introText', el)}>
        <h1 ref={(el: HTMLHeadingElement): void => targets.set('hi', el)}>Hi!</h1>
        <WriteWithCursor
          wrapperRef={(el: HTMLHeadingElement): void => targets.set('nice', el)}
          ref={(el: HTMLHeadingElement): void => targets.set('niceSplit', [el])}
          textWrapper={(children: React.ReactNode): JSX.Element => <h2>{children}</h2>}>
      ...
      <Layer ref={(el: HTMLDivElement): void => targets.set('itsMe', el)}>
        <Flex direction={"column"} justifyContent={'center'}>
          <Polaroid ref={(el: HTMLHeadingElement): void => targets.set('picture', el)} title={<h4><span>⬆️</span> THIS GUY <span>⬆️</span></h4>}>
            <DynamicImage path="wallpaper/me.jpg" />
          </Polaroid>
          <WriteWithCursor
            wrapperRef={(el: HTMLHeadingElement): void => targets.set('name', el)}
            ref={(el: HTMLHeadingElement): void => targets.set('nameSplit', [el])}
            cursorRef={(el: HTMLHeadingElement): void => targets.set('nameSplitCursor', el)}
            textWrapper={(children: React.ReactNode): JSX.Element => <h2>{children}</h2>}>
            My name is Casimir
          </WriteWithCursor>
          ...
          <WriteWithCursor
            wrapperRef={(el: HTMLHeadingElement): void => targets.set('portfolio', el)}
            ref={(el: HTMLHeadingElement): void => targets.set('portfolioSplit', [el])}
            cursorRef={(el: HTMLHeadingElement): void => targets.set('portfolioSplitCursor', el)}
            textWrapper={(children: React.ReactNode): JSX.Element => <h3>{children}</h3>}>
...
export function OpeningCredits({ position }: { position: string }): JSX.Element {
  return (
    <Timeline
    ...
    >
      <InOut
        positions={['fadeInRoot', 'fadeOutRoot']}
        target={'sceneRoot'}
      />

      <InOut
        positions={['fadeInRoot', 'introText']}
        target={'hi'}
      />

      <InOut
        positions={['fadeInRoot', 'introduction']}
        target={'introText'}
      />
      <InOut
        positions={['introText', 'introText+=1']}
        target={'nice'}
      />
      <Write
        position={'introText'}
      ...
      <InOut
        positions={['introText+=1', 'outroText']}
        target={'itsMe'}
      />
      <InOut
        positions={['introText+=1', 'outroText']}
      ...
      <InOut
        positions={['myPortfolio', 'outroText']}
        target={'portfolio'}
      />
      <Write
        position={'picture+=1'}
        target={'portfolioSplit'}
      />
      <AnimateCursor
