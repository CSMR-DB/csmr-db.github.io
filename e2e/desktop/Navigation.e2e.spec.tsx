...
describe('Navigation (both WASDNav & IONavigator)', (): void => {
  ...
  describe.each([
    ...
    it(`should navigate to /${route} on keypress of ${key}`, async (): Promise<
      void
    > => {
      await page.evaluate(
        async ({ passedKey }: { passedKey: string }): Promise<void> => {
          window.dispatchEvent(
            new KeyboardEvent('keydown', { key: passedKey.toLocaleLowerCase() })
          )
        },
        { passedKey: key }
      )
      ...
      await page.screenshot({
        path: path.join(
          __dirname,
          `/screenshots/${
            route !== '' ? route.toLocaleLowerCase() : 'homepage'
          }.png`
        ),
        fullPage: true,
      })
